...
-- Задача 2: выбрать фотографии (filename) пользователя с email: arlo50@example.org.ID типа медиа, 
...
SELECT media.filename AS 'Название файла', COUNT(likes.user_id) AS 'Количество лайков'
FROM media 
JOIN likes
...
-- Задача 6: Список медиафайловпользователей, указав название типа медиа (id, filename, name_type)(используя JOIN)
SELECT media.id, media.filename AS 'Название файла', mt.name_type AS 'Тип файла'
FROM media 
...
CREATE VIEW messages_user AS
SELECT body
FROM messages
WHERE from_user_id = 1 OR to_user_id = 1;

SELECT * FROM messages_user;
...
WITH friend_user AS
    ...
    SELECT initiator_user_id 
	FROM friend_requests 
	WHERE target_user_id IN (SELECT * FROM friend_user) AND status = 'approved'
	UNION 
	SELECT target_user_id 
	FROM friend_requests 
	WHERE initiator_user_id IN (SELECT * FROM friend_user) AND status = 'approved';
